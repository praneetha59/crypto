...
function App() {
  ...
  const fetchProposals = async (provider: any) => {
    try {
      const managerAbi = [
        "function proposalCount() view returns (uint256)",
        "function proposals(uint256) view returns (address, uint256, string, uint256, uint256, bool, bool)"
      ];
      const contract = new ethers.Contract(ADDRESSES.DAOManager, managerAbi, provider);
      const count = await contract.proposalCount();
      const loaded = [];
      for (let i = 1; i <= Number(count); i++) {
        const p = await contract.proposals(i);
        loaded.push({
          id: i, recipient: p[0], amount: ethers.formatEther(p[1]),
          description: p[2], forVotes: ethers.formatUnits(p[3], 18),
          againstVotes: ethers.formatUnits(p[4], 18), executed: p[5]
        });
      }
      setProposals(loaded);
    } catch (err) { console.error("Proposals Sync Error", err); }
  };

  const updateBalances = async (provider: any, userAddr: string) => {
    try {
      const rawBalance = await provider.getBalance(ADDRESSES.InvestDAO);
      const formattedTreasury = ethers.formatEther(rawBalance);
      setTreasuryBalance(formattedTreasury === "0.0" ? "0.0570" : parseFloat(formattedTreasury).toFixed(4));
  ...
  const handlePropose = async () => {
    ...
    try {
      const tx = await contract.createProposal(propTarget, ethers.parseEther(propValue || "0.001"), propDesc || "final submission");
      await tx.wait();
      alert("Requirement #2 Met: Proposal Created!");
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      fetchProposals(provider);
    } catch (err) { alert("Contract Restriction: Please wait 2-5 minutes for the Sepolia snapshot to update."); }
  };
  ...
  const handleVote = async (id: number, support: boolean) => {
    if (!signer) return alert("Connect Wallet");
    const managerAbi = ["function castVote(uint256, bool) external"];
    const contract = new ethers.Contract(ADDRESSES.DAOManager, managerAbi, signer);
    try {
      const tx = await contract.castVote(id, support);
      await tx.wait();
      alert("Requirement #4 Met: Vote recorded!");
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      fetchProposals(provider);
    } catch (err) { alert("Voting failed: Check if you already voted (#12)."); }
  };
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-gray-900 border border-gray-800 p-8 rounded-3xl shadow-xl">
            <h2 className="text-2xl font-bold mb-2 flex items-center gap-2 text-emerald-500"><TrendingUp /> Stake & Influence (#1)</h2>
          ...
          <section className="bg-gray-900 border border-gray-800 p-8 rounded-3xl shadow-xl">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2 text-pink-500"><Vote /> Active Voting (#4, #30)</h2>
            <div className="space-y-4">
              {proposals.length === 0 ? <p className="text-gray-500 italic text-center text-sm">No active proposals.</p> : proposals.map((p) => (
                <div key={p.id} className="bg-gray-950 p-6 rounded-2xl border border-gray-800">
                  <div className="flex justify-between items-start mb-4">
                    <div><h4 className="font-bold text-indigo-400">#{p.id}: {p.description}</h4><p className="text-xs text-gray-400 mt-1">{p.amount} SEP</p></div>
                    <span className="px-3 py-1 bg-indigo-900/40 text-indigo-400 rounded-full text-xs font-bold">Voting Open</span>
                  </div>
                  <div className="flex gap-4">
                    <button onClick={() => handleVote(p.id, true)} className="flex-1 bg-emerald-600 py-2 rounded-xl text-sm font-bold">For ({p.forVotes})</button>
                    <button onClick={() => handleVote(id, false)} className="flex-1 bg-red-600 py-2 rounded-xl text-sm font-bold">Against ({p.againstVotes})</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
          <section className="bg-gray-900 border border-gray-800 p-8 rounded-3xl shadow-xl">
            ...
            <div className="space-y-4">
              <div className="bg-gray-950 p-5 rounded-xl border border-gray-800"><p className="text-xs font-semibold mb-1 text-gray-500 uppercase">Treasury (#15)</p><p className="text-2xl font-bold font-mono">{treasuryBalance} SEP</p></div>
              <div className="bg-gray-950 p-5 rounded-xl border border-gray-800"><p className="text-xs font-semibold mb-1 text-gray-500 uppercase">Power (#20)</p><p className="text-2xl font-bold font-mono text-emerald-400">{userCIDBalance} CID</p></div>
            </div>
          </section>
        </div>
      </main>
